...
class Comment extends DatabaseObject {
  ...
  protected static $db_fields=array('id', 'post_id', 'created', 'author', 'body', 'email');
  ...
  // "new" is a reserved word so we use "make" (or "build")
	public static function make($photo_id, $author="Anonymous", $body="", $email="") {
    if(!empty($photo_id) && !empty($author) && !empty($body) && !empty($email)) {
		$comment = new Comment();
	    $comment->post_id = (int)$photo_id;
    ...
    $sql .= " WHERE post_id=" .$database->escape_value($photo_id);
